<x-patient.playout title="Commandes | Patient" :breadcrum="['Commandes']" activePage="3">
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="intro-y col-span-12 overflow-auto 2xl:overflow-visible">
            <table class="table table-report -mt-2">
                <thead>
                <tr>
                    <th class="whitespace-nowrap">ID COMMANDE</th>
                    <th class="text-center whitespace-nowrap">STATUT</th>
                    <th class="text-right whitespace-nowrap">
                        <div class="pr-16">TOTAL COMMANDE</div>
                    </th>
                ...
                <tr class="intro-x commande-row" data-id="{{ $com->id }}" data-statut="{{ $com->statut }}">

                    <td class="w-40">
                        <div class="font-medium whitespace-nowrap">{{ $com->id }}</div>
                    </td>
                        ...
                        <div class="flex items-center justify-center whitespace-nowrap
                            @switch($com->statut)
                                ...
                                @case('livre')
                                 ...
                                 @break
                            @endswitch
                        "> <i data-lucide="check-square" class="w-4 h-4 mr-2"></i> {{ $com->statut }} </div>
                    </td>
                    <td class="w-40 text-right">
                        <div class="pr-16">{{ $com->total }} DH</div>
                    </td>
                    <td class="table-report__action">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center text-primary whitespace-nowrap mr-5" href="{{ route('patient-commande', [$com->id]) }}"> <i data-lucide="check-square" class="w-4 h-4 mr-1"></i> Voir les Détails </a>
    ...
    <script>
        ...
        let currentPage = 1;
        let rowsPerPage = 15;
        let allRows = Array.from(document.querySelectorAll('.commande-row'));
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('[data-tw-toggle="modal"][data-tw-target="#delete-confirmation-modal"]').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    commandeIdToDelete = row.dataset.id;
                });
            });
